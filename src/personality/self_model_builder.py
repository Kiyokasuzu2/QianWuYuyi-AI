...
class SelfModelBuilder:
    ...
    def build(
        ...
        identity_name = (
            base_identity
            or IDENTITY_CORE.get("name", "")
        )

        capability_limitations = (
            capability_limitations or []
        )
        ...
        for r in history.get_high_confidence(0.7):
            ...
            momentum = self._get_record_momentum(
                r,
                trait_states
            )
        ...
        return {
            ...
            "self_description":
                self._generate_self_description(
                    identity_name,
                    history
                ),
            ...
            "growth_narratives":
                self._extract_growth_narratives(
                    history
                ),


            "current_traits":
                self._collect_current_traits(
                    trait_states
                ),
    ...
    def _get_record_momentum(
        ...
        for dim in record.get(
            "affected_dimensions",
            []
        ):

            state = trait_states.get(
                dim
            )
    ...
    def _detect_tensions(
        ...
        traits=self._collect_current_traits(
            states
        )
    ...
    def _calc_self_understanding(
        ...
        awareness=min(
            ...
            +
            len(
                history.get_high_confidence(0.7)
            )
            *
            0.04,
            1
